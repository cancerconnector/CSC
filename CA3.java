...
public class CA3 {
	...
	int size = 100; // Size of the system lattice
	...
	int timestep=0; // Current Number of timesteps in the simulation
        ...
        int dataWriteStep = 10; // how often we write down the data
        int dataReportStep = 10; // how often we write down the data

	// Model Parameters
	float initOxygen=0.2f;
	float[] proliferation = {
	 0.5f,  /* healthy cells */ 
	 0.005f, /* stem cells*/
    ...
    float consumptionBasal=   0.0001f; //**********************
	float consumptionDivision= consumptionBasal; // The Oxygen consumption for cells that proliferate **was 5, changing to 4 on 18 december 2013,JGS
	...
	final int[] convertCoordinates(int x, int y)
       {
               // This method converts the coordinates so they take on account the boundaries of the lattice
	...
	void resetCells ()
	{
		...
		//Cells[centre][centre]=1; // just dealing with healthy cells now
	}	
		...
		while (cellList.size() != 0) {
			...
			// Do we have space for division or motility ?
				...
				if (proliferation[cell]>=random.nextFloat()) {// If tossing the coin we are to proliferate...
				    if ((cell==0) || (cell==1) || ((cell==2) && (Age[rI][rJ]<maxProDivisions))) // AND the cell is stem or progenitor ... ********HEALTHY DIVISION********
...
//{
    ...
    // alternative version where healthy cells are NOT open sites - for validation and homeostasis
{
	// We assume that necrotic material counts as neighbour? NOTE: MOORE NEIGHBORHOOD
	int total=0;
	int[] p = new int [2];
	...
	p=convertCoordinates (x+1,y-1);
	if (Cells[p[0]][p[1]]==4) total++; //making into a logical OR to include possibility of cells dividing into areas of necrosis cells=4
	p=convertCoordinates (x+1,y);
...
int[] findEmptySite (int x, int y)
{
	...
	//if ((Cells[tp1[0]][tp1[1]]==0) || (Cells[tp1[0]][tp1[1]]==4)) vacantSites.add(tp1);
	//tp2=convertCoordinates (x+1,y);
	//if ((Cells[tp2[0]][tp2[1]]==0) || (Cells[tp2[0]][tp2[1]]==4)) vacantSites.add(tp2);
	//tp3=convertCoordinates (x+1,y+1);
	//if ((Cells[tp3[0]][tp3[1]]==0) || (Cells[tp3[0]][tp3[1]]==4)) vacantSites.add(tp3);
	//tp4=convertCoordinates (x,y-1);
	//if ((Cells[tp4[0]][tp4[1]]==0) || (Cells[tp4[0]][tp4[1]]==4)) vacantSites.add(tp4);
	//tp5=convertCoordinates (x,y+1);
	//if ((Cells[tp5[0]][tp5[1]]==0) || (Cells[tp5[0]][tp5[1]]==4)) vacantSites.add(tp5);
	//tp6=convertCoordinates (x-1,y-1);
	//if ((Cells[tp6[0]][tp6[1]]==0) || (Cells[tp6[0]][tp6[1]]==4)) vacantSites.add(tp6);
	//tp7=convertCoordinates (x-1,y);
	//if ((Cells[tp7[0]][tp7[1]]==0) || (Cells[tp7[0]][tp7[1]]==4)) vacantSites.add(tp7);
	//tp8=convertCoordinates (x-1,y+1);
	//if ((Cells[tp8[0]][tp8[1]]==0) || (Cells[tp8[0]][tp8[1]]==4)) vacantSites.add(tp8);
