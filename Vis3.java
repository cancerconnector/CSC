...
public class Vis3 extends JApplet implements ActionListener{
	public static String version="8 Oct 2013";
	...
	int numParticles=0;
	int[][] OriginalMatrix;
	int mag = 2; // Magnification factor (WAS 2)
        int imageWriteStep = 50; // how often we save the image file
	public CA3 ca;
	...
	public Vis3 (JButton b, JLabel l, JCheckBox _cb, float SCSymmetricDivBalance, int maxDivisions, float densityV)
	{
		fc = new JFileChooser();
		button = b;
		label = l;
	...
	public BufferedImage getCells()
	{
		...
		for (int i=0;i<ca.size;i++)
			for (int j=0;j<ca.size;j++) {
				int val=0;
				if (lattice[i][j]==0) val=Color.yellow.getRGB(); // Healthy cell
				...
				else if (lattice[i][j]==4) val=Color.white.getRGB(); // Necrotic
	...
	public static void main(String args[]) {
		...
		System.err.println ("# Vis version:"+Vis3.version);
		...
		JFrame f = new JFrame("CSCTherapy CA Visualisation "+CA.version);
		...
		Vis3 m = new Vis3 (play,label,cb,SCSymmetricDivBalance,maxDivisions,densityV);
		...
		while ((ts!=maxTS) && (m.ca.simulationFinished==false)) {
			if (m.play) {
				m.nextTimeStep();
